    ...
    public class Agent
    {
        ...
        public void computeNewVelocity()
        {
            ...
            for (int i = 0; i < obstacleNeighbors_.Count; ++i)
            {
                ...
                var v1 = -relativePosition1;
                var v2 = obstacleVector;
                float s = (v1.x * v2.x + v1.y * v2.y) / RVOMath.absSq(obstacleVector);
                ...
                var vl = velocity_ - leftCutOff;
                var vr = velocity_ - rightCutOff;
                float t = obstacle1 == obstacle2 ? 0.5f : (vl.x * cutOffVector.x + vl.y * cutOffVector.y) / RVOMath.absSq(cutOffVector);
                float tLeft = vl.x * leftLegDirection.x + vl.y * leftLegDirection.y;
                float tRight = vr.x * rightLegDirection.x + vr.y * rightLegDirection.y;

                if ((t < 0.0f && tLeft < 0.0f) || (obstacle1 == obstacle2 && tLeft < 0.0f && tRight < 0.0f))
                {
                    /* Project on left cut-off circle. */
                    float2 unitW = math.normalize(velocity_ - leftCutOff);
                ...
                else if (t > 1.0f && tRight < 0.0f)
                {
                    ...
                    float2 unitW = math.normalize(velocity_ - rightCutOff);
            ...
            for (int i = 0; i < agentNeighbors_.Count; ++i)
            {
                ...
                if (distSq > combinedRadiusSq)
                {
                    ...
                    float dotProduct1 = w.x * relativePosition.x + w.y * relativePosition.y;
                    ...
                    else
                    {
                        ...
                        float dotProduct2 = relativeVelocity.x * line.direction.x + relativeVelocity.y * line.direction.y;
        ...
        private bool linearProgram1(IList<Line> lines, int lineNo, float radius, float2 optVelocity, bool directionOpt, ref float2 result)
        {
            ...
            {
                /* Optimize direction. */
                if (optVelocity.x * lines[lineNo].direction.x + optVelocity.y * lines[lineNo].direction.y > 0.0f)
                {
                    /* Take right extreme. */
            ...
            else
            {
                ...
                var diff = optVelocity - lines[lineNo].point;
                float t = lines[lineNo].direction.x * diff.x + lines[lineNo].direction.y * diff.y;
                ...
                if (RVOMath.det(lines[i].direction, lines[i].point - result) > distance)
                {
                    ...
                    for (int j = numObstLines; j < i; ++j)
                    {
                        ...
                        if (RVOMath.fabs(determinant) <= RVOMath.RVO_EPSILON)
                        {
                            ...
                            if (lines[i].direction.x * lines[j].direction.x + lines[i].direction.y * lines[j].direction.y > 0.0f)
                            {
